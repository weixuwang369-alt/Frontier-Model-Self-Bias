...
providers = sorted({m.provider.value for m in config.roster})
c4.markdown(ui.stat_list("Providers", providers), unsafe_allow_html=True)
st.dataframe(
    [
        {
            "stage": stage,
...
if st.button("Start the run", type="primary", disabled=launch_disabled):
    ...
    # One-click: compute the analysis right after the run so results need no terminal step.
    from selfbias.analysis import analyze as run_analysis

    st.divider()
    ui.eyebrow("Analysis")
    try:
        with st.spinner("Crunching the numbers (bootstrap confidence intervals, onset lengths)..."):
            report = run_analysis(
                config,
                data_root=str(ROOT / "data"),
                n_boot=config.analysis.bootstrap_iterations,
            )
        st.success(
            "Analysis complete. Open the Results page (left sidebar) and pick this run at "
            "the bottom to see your charts."
        )
        onsets = report.get("onsets", {})
        labels = {
            "recognition_accuracy": "Recognizes itself at",
            "attribution_f1": "Author guessable at",
            "hspp_r_self": "Starts favoring itself at",
        }
        cols = st.columns(3)
        for col, key in zip(cols, labels, strict=True):
            onset = onsets.get(key)
            col.metric(labels[key], f"{onset} tokens" if onset else "not yet")
    except Exception as exc:  # noqa: BLE001 - the run succeeded; only analysis failed
        st.warning(
            "The run finished and its data is saved, but the analysis step could not "
            f"complete: {exc}. You can retry it in a terminal with "
            "`uv run selfbias analyze <your config>`."
        )
