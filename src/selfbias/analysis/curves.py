...
def recognition_df(paths: DataPaths, keep_gen_ids: set[str] | None = None) -> pd.DataFrame:
    """Controlled-length recognition probes as (judge, prompt, bin, correct).

    ``keep_gen_ids`` restricts to one run's generations (probes over others are skipped).
    """
