...
def attribution_df(paths: DataPaths, keep_gen_ids: set[str] | None = None) -> pd.DataFrame:
    """Controlled-length generations as (model, prompt, bin, text) for attribution.

    ``keep_gen_ids`` restricts to one run's generations (others are excluded).
    """
    ...
    rows = [
        ...
        and g.text.strip()
        and (keep_gen_ids is None or g.gen_id in keep_gen_ids)
    ]
...
def _oof_predictions(d: pd.DataFrame) -> pd.DataFrame | None:
    ...
    preds = np.full(len(d), None, dtype=object)
    gkf = GroupKFold(n_splits=n_splits)
    for train, test in gkf.split(X, y, groups=groups):
        # A prompt-grouped fold can leave the training split single-class (common with few
        # models/prompts); the classifier can't train on it, so skip that fold's test rows.
    ...
    if not scored.any():  # no fold could be trained -> not enough data for this bin
        return None
    return pd.DataFrame({"prompt": groups[scored], "true": y[scored], "pred": preds[scored]})
