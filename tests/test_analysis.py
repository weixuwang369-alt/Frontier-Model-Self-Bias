...
def test_analyze_end_to_end_on_mock(mock_config, tmp_path):
    ...
    obs = build_rb_observations(default_data_paths(data_root, run_id=mock_config.run_id()))
...
def test_analyze_is_scoped_to_its_run(mock_config, config_dict, tmp_path):
    """A second experiment sharing the data dir must not leak into the first run's report."""
    ...
    # A different run (distinct model strings + seed) writing to the SAME shared files.
    other = ExperimentConfig.model_validate(
        {
            **config_dict,
            "run": {**config_dict["run"], "name": "other-run", "seed": 999},
            "roster": [
                {"slot": "x", "provider": "mock", "model": "mock-x-frontier", "family": "x"},
                {"slot": "y", "provider": "mock", "model": "mock-y-frontier", "family": "y"},
            ],
        }
    )
    Pipeline(other, data_root=data_root, pricing_path="config/pricing.yaml").run()

    report = analyze(mock_config, data_root=data_root, n_boot=50)
    assert report["scope"] == "manifest"
    roster = {m.model for m in mock_config.roster}
    seen = {r["judge"] for r in report["overestimation_matrix"]}
    seen |= {r["generator"] for r in report["overestimation_matrix"]}
    assert seen and seen <= roster  # only this run's models; no "mock-x/y" leakage
