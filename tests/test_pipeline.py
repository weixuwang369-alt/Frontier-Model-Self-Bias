...
def test_rows_are_valid_and_anonymized(mock_config, tmp_path):
    pipe = _pipeline(mock_config, tmp_path)
    pipe.run()
    paths = default_data_paths(tmp_path / "data", run_id=mock_config.run_id())
...
def test_shared_data_root_isolated_by_seed(config_dict, tmp_path):
    ...
    # Per-run dirs keep the two runs' generations fully separate on disk.
    gens_a = JsonlStore(
        default_data_paths(root, run_id=cfg_a.run_id()).generations / "generations.jsonl"
    ).read_all(Generation)
    gens_b = JsonlStore(
        default_data_paths(root, run_id=cfg_b.run_id()).generations / "generations.jsonl"
    ).read_all(Generation)
    ids_a = {g.gen_id for g in gens_a if g.truncation_of is None}
    ids_b = {g.gen_id for g in gens_b if g.truncation_of is None}
    assert len(ids_a) == controlled and len(ids_b) == controlled
    assert ids_a.isdisjoint(ids_b)  # no cross-run reuse
